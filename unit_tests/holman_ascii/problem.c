/**
 * Same as unit_tests/holman_spk, but using the ASCII-derived binary planets file
 * (linux_p1550p2650.440) instead of an SPK planets kernel.
 *
 * This ensures we exercise the full ASCII-path plumbing:
 * - `assist_ephem_create` selects FILE_FORMAT_ASCII_BIN
 * - `ephem->planets_calc` dispatches through the ephem wrapper
 * - asteroid masses are populated so ASTEROIDS forces match the SPK mode
 *
 * The expected final state is from NASA Horizons (see holman/problem.c).
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
int main(int argc, char* argv[]){
    ...
    reb_simulation_integrate(r, 8446.5);
