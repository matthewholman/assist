/**
 * Same as unit_tests/apophis_spk, but using the ASCII-derived binary (.440/.441) planets file
 * (linux_p1550p2650.440) instead of an SPK planets kernel.
 *
 * This catches regressions where ASCII planets mode accidentally disables
 * asteroid perturbations (e.g. missing MAxxxx mass join), which can move the
 * trajectory by ~60 km over a year for Apophis.
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
